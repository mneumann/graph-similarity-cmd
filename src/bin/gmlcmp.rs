...
use graph_neighbor_matching::{ScoreNorm, WeightedNodeColors, SimilarityMatrix};
...
fn main() {
    let fa = env::args().nth(1).unwrap();
    let fb = env::args().nth(2).unwrap();

    let a = load_graph(&fa);
    let b = load_graph(&fb);

    let score = score_graphs(&a, &b, 100, 0.01, false);
    println!("{:.2}", score);
}
